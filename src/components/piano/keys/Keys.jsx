...
export const Keys = () => {
  const { instrument, volume, audioContext, destination } =
    useContext(InstrumentContext)

  useEffect(() => {
    const handleKeyDown = (event) => {
      const keyPressed = event.key.toLowerCase()
      const keyData = instrument.find(
        (tile) => tile.key.toLowerCase() === keyPressed,
      )
      ...
      if (!keyData.sound.ended) {
        ...
        newSound
          .play()
          .then(() => console.log('Reproduciendo:', keyData.sound))
  ...
  const handleClick = (sound) => {
    ...
    if (!sound.ended) {
      ...
      newSound
        .play()
        .then(() => console.log('Reproduciendo:', newSound))
        .catch((error) => console.error(error))
    } else {
      sound
        .play()
        .then(() => console.log('Reproduciendo:', sound))
        .catch((error) => console.error(error))
    }
  }
      ...
      {instrument.map((tile) => {
        return (
          <div
            ...
            style={
              tile.type === 'black' ? { left: `${tile.position * 52}px` } : {}
            }
